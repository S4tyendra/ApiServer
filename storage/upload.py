...
async def upload_file(drive_service, file: UploadFile, FOLDER_ID):
    file.file.seek(0, 2)  # Move the cursor to the end of the file to get the size
...
async def upload(request: Request, file: UploadFile = File(...)):
    ...
    if session_data:
        ...
        if user:
            ...
            if not drive_id:
                drive_id = create_folder_and_get_id(user.get("_id"), "1p90vuxE8jp7mBwW63qj7rIwpiCuaYMEP")
                await db.users.update_one({'_id': user.get('_id')}, {'$set': {'drive_id': drive_id}})
            drive_service = authenticate()
            
            rand_file_id = f"{user.get('_id')}_{token_hex(16)}"
            uploaded_file_id, file_size = await upload_file(drive_service, file, drive_id)
