...
async def login(client, message):
    await message.reply("Click below button to get login Link.", reply_markup=InlineKeyboardMarkup(
        [
...
async def login_callback(client, query):
    ...
    await query.message.edit_text("Please login to the bot by clicking the button below.", reply_markup=InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("🔑 Login", url=f"https://apiauth.devh.in/auth/connect?app=telegram@devh.in?app=telegram&_hash={_hash}=&state={state}")]
        ]
    ))